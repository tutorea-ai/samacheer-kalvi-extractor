...
# Maths subject names
MATHS_NAMES = ["maths", "math", "mathematics"]

# Biology subject names
BIOLOGY_NAMES = ["biology"]
...
class ContentBridge:
    ...
    def __init__(self):
        ...
        # ── Load Maths curriculum ─────────────────────────────────────────────
        self.maths_curriculum = self._load_maths_curriculum()
    ...
    def _load_maths_curriculum(self) -> dict:
        """Load Maths curriculum JSON."""
        try:
            maths_path = settings.BASE_DIR / "data" / "curriculum" / "subjects" / "english-medium" / "maths.json"
            with open(maths_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            print(f"✅ Bridge Maths curriculum loaded: {maths_path}")
            return data
        except FileNotFoundError:
            print(f"⚠️  Bridge: Maths curriculum not found — Maths deployment will fail")
            return {}
        except Exception as e:
            print(f"❌ Bridge Error loading Maths curriculum: {e}")
            return {}

    def _load_biology_curriculum(self) -> dict:
        """Load Biology curriculum JSON."""
        try:
            biology_path = settings.BASE_DIR / "data" / "curriculum" / "subjects" / "english-medium" / "biology.json"
            with open(biology_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            print(f"✅ Bridge Biology curriculum loaded: {biology_path}")
            return data
        except FileNotFoundError:
            print(f"⚠️  Bridge: Biology curriculum not found — Biology deployment will fail")
            return {}
        except Exception as e:
            print(f"❌ Bridge Error loading Biology curriculum: {e}")
            return {}

    def _load_pure_science_curriculum(self) -> dict:
        """Load Pure Science curriculum JSON."""
        try:
            ps_path = settings.BASE_DIR / "data" / "curriculum" / "subjects" / "english-medium" / "pure_science.json"
            with open(ps_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            print(f"✅ Bridge Pure Science curriculum loaded: {ps_path}")
            return data
        except FileNotFoundError:
            print(f"⚠️  Bridge: Pure Science curriculum not found")
            return {}
        except Exception as e:
    ...
    def is_already_deployed(self, metadata: dict) -> bool:
        ...
        if subject in SOCIAL_SCIENCE_NAMES:
            ...
            return self._is_maths_deployed(metadata)
        elif subject in BIOLOGY_NAMES:
    ...
    def _is_maths_deployed(self, metadata: dict) -> bool:
        """Check deployment status for Maths lessons."""
        lesson_id = self._resolve_maths_lesson_id(metadata)
        if not lesson_id:
            return False
        ...
        base = (self.target_base / "backend" / "data" / "subjects" /
                "english-medium" / "maths" / class_str)
    ...
    def _is_biology_deployed(self, metadata: dict) -> bool:
        """Check deployment status for Biology lessons."""
        lesson_id = self._resolve_biology_lesson_id(metadata)
        if not lesson_id:
            return False

        class_str  = str(metadata["class_num"])
        discipline = metadata.get("discipline", "botany").lower()
        base = (self.target_base / "backend" / "data" / "subjects" /
                "english-medium" / "biology" / class_str)

        content_path = base / "content" / "term0" / discipline / lesson_id / "index.html"
        qa_path      = base / "qa"      / "term0" / discipline / lesson_id / "index.html"
        lp_path      = base / "lp"      / "term0" / discipline / lesson_id / "index.html"
    ...
    def _is_pure_science_deployed(self, metadata: dict) -> bool:
        """Check deployment status for Pure Science lessons."""
        lesson_id = self._resolve_pure_science_lesson_id(metadata)
        if not lesson_id:
            return False
        ...
        discipline = metadata.get("discipline", "botany").lower()
        base = (self.target_base / "backend" / "data" / "subjects" /
                "english-medium" / "pure-science" / class_str)
    ...
    def _resolve_maths_lesson_id(self, metadata: dict) -> str | None:
        """Maps (class, term, unit) → lesson_id for Maths."""
        class_str = str(metadata["class_num"])
        unit_num  = metadata.get("unit", 1)
        class_num = int(class_str)
        term_key  = "term0" if class_num >= 8 else f"term{metadata.get('term', 1)}"
        ...
        try:
            if class_str not in self.maths_curriculum:
                print(f"❌ Bridge Maths: Class '{class_str}' not in Maths curriculum")
                return None

            term_data = self.maths_curriculum[class_str].get(term_key)
            if not term_data:
            ...
            if not lesson_data:
                print(f"❌ Bridge Maths: Unit {unit_num} not found")
                return None

            lesson_id = lesson_data["id"]
            print(f"🌉 Bridge Maths: Resolved → '{lesson_id}'")
            return lesson_id

        except Exception as e:
            print(f"❌ Bridge Maths: Lesson ID resolution failed: {e}")
            return None

    def _resolve_biology_lesson_id(self, metadata: dict) -> str | None:
        """Maps (class, term, discipline, chapter) → lesson_id for Biology."""
        class_str  = str(metadata["class_num"])
        discipline = metadata.get("discipline", "botany").lower().strip()
        unit_num   = metadata.get("unit", 1)
        class_num  = int(class_str)
        term_key   = "term0"

        try:
            if class_str not in self.biology_curriculum:
                print(f"❌ Bridge Biology: Class '{class_str}' not in Biology curriculum")
                return None

            term_data = self.biology_curriculum[class_str].get(term_key)
            if not term_data:
                print(f"❌ Bridge Biology: '{term_key}' not found for class {class_str}")
                return None

            disc_lessons = term_data.get(discipline)
            if not disc_lessons:
                print(f"❌ Bridge Biology: Discipline '{discipline}' not found")
                return None

            lesson_data = next(
                (l for l in disc_lessons if l.get("chapter") == unit_num),
                None
            )
            if not lesson_data:
                print(f"❌ Bridge Biology: Chapter {unit_num} not found in {discipline}")
                return None

            lesson_id = lesson_data["id"]
            print(f"🌉 Bridge Biology: Resolved → '{lesson_id}'")
            return lesson_id

        except Exception as e:
            print(f"❌ Bridge Biology: Lesson ID resolution failed: {e}")
            return None

    def _resolve_pure_science_lesson_id(self, metadata: dict) -> str | None:
        """Maps (class, term, discipline, chapter) → lesson_id for Pure Science."""
        class_str  = str(metadata["class_num"])
        discipline = metadata.get("discipline", "botany").lower().strip()
        unit_num   = metadata.get("unit", 1)
        term_key   = "term0"
        ...
        try:
            if class_str not in self.pure_science_curriculum:
                print(f"❌ Bridge Pure Science: Class '{class_str}' not in curriculum")
                return None

            term_data = self.pure_science_curriculum[class_str].get(term_key)
            if not term_data:
                print(f"❌ Bridge Pure Science: '{term_key}' not found for class {class_str}")
                return None

            disc_lessons = term_data.get(discipline)
            if not disc_lessons:
                print(f"❌ Bridge Pure Science: Discipline '{discipline}' not found")
                return None
            ...
            if not lesson_data:
                print(f"❌ Bridge Pure Science: Chapter {unit_num} not found in {discipline}")
                return None

            lesson_id = lesson_data["id"]
            print(f"🌉 Bridge Pure Science: Resolved → '{lesson_id}'")
            return lesson_id
