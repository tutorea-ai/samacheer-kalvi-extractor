...
Usage:
    extractor = PureScienceEpubExtractor(epub_dir)
...
class PureScienceEpubExtractor:
    ...
    def extract(self, unit: int) -> str | None:
        ...
        if not self.preprocessor.combined_tagged_path.exists():
            print(f"[PureScienceEpubExtractor] Preprocessing {self.epub_dir.name}...")
            success = self.preprocessor.prepare()
            if not success:
                print(f"[PureScienceEpubExtractor] ❌ Preprocessing failed")
                return None

        text = self.preprocessor.extract("unit", unit)

        if text is None:
            print(f"[PureScienceEpubExtractor] ❌ Unit {unit} not found — fallback to pdfplumber")
