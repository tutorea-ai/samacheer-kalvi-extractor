...
Uses EpubPreprocessor — same pattern as SocialMathsEpubExtractor.
...
Usage:
    extractor = MathsEpubExtractor(epub_dir)
...
class MathsEpubExtractor:
    ...
    def extract(self, unit: int) -> str | None:
        ...
        if not self.preprocessor.combined_tagged_path.exists():
            print(f"[MathsEpubExtractor] Preprocessing {self.epub_dir.name}...")
            success = self.preprocessor.prepare()
            if not success:
                print(f"[MathsEpubExtractor] ❌ Preprocessing failed")
                return None

        text = self.preprocessor.extract("unit", unit)

        if text is None:
            print(f"[MathsEpubExtractor] ❌ Unit {unit} not found — fallback to pdfplumber")
