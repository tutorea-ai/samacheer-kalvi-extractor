...
Usage:
    extractor = BiologyEpubExtractor(epub_dir)
...
class BiologyEpubExtractor:
    ...
    def extract(self, unit: int) -> str | None:
        ...
        if not self.preprocessor.combined_tagged_path.exists():
            print(f"[BiologyEpubExtractor] Preprocessing {self.epub_dir.name}...")
            success = self.preprocessor.prepare()
            if not success:
                print(f"[BiologyEpubExtractor] ❌ Preprocessing failed")
                return None

        text = self.preprocessor.extract("unit", unit)

        if text is None:
            print(f"[BiologyEpubExtractor] ❌ Unit {unit} not found — fallback to pdfplumber")
