"""
biology.py  (QA Builder — Grade 6/7)
--------------------------------------
...
Key rules:
  ...
  - Biology focus: plants, animals, body parts, health
...
- Biology focus: observation, classification, structure-function, observation
"""


class BiologyQA67Builder:

    def __init__(self):
        self.client = anthropic.Anthropic(api_key=settings.ANTHROPIC_API_KEY)
        self.model  = settings.ANTHROPIC_MODEL
        print(f"✅ Biology QA Builder (67) v1.0 initialized — model: {self.model}")
    ...
    def generate(self, text: str, metadata: dict) -> Optional[str]:
        ...
        print(f"      [Biology QA 67 v1.0] Generating: {lesson_title}")
        parts = []
        ...
        for call_num, (method, label) in enumerate([
            ...
            print(f"      [Biology QA 67] Call {call_num}/4: {label}...")
        ...
        if not parts:
            return None
        combined = "\n\n".join(parts)
        print(f"      [Biology QA 67 v1.0] ✅ Complete — {len(combined)} chars")
        return combined

    def _call_mcq(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate ONLY MCQ Q1–Q25 for Class 6/7 Biology.

{ANSWER_FORMAT_RULES}
{AGE_NOTE}

Chapter: {lesson_title} | Class {class_num} | Unit {unit} | Biology
{disc_context}
...
Start Q1. End Q25."""
            ...
            print(f"❌ Biology QA 67 MCQ error: {e}")
            return None

    def _call_fill_blanks(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate ONLY Fill Blanks Q26–Q50 for Class 6/7 Biology.
...
Start Q26. End Q50."""
            ...
            print(f"❌ Biology QA 67 Fill Blanks error: {e}")
            return None
...
Class 6/7 Biology. Age-appropriate. No repeats from Q1–Q50.
...
Biology Match Pairs:
- Structure → Function / Plant Part → Role / Animal → Class / Disease → Cause / Organ → Function
...
Start Q51. End Q75. Raw HTML only."""
            ...
            print(f"❌ Biology QA 67 Statement+Match error: {e}")
            return None

    def _call_descriptive(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate 2-mark (Q76–Q88) and 5-mark (Q89–Q100) for Class 6/7 Biology.
...
---"""
            ...
            print(f"❌ Biology QA 67 Descriptive error: {e}")
            return None


biology_qa_67_builder = BiologyQA67Builder()