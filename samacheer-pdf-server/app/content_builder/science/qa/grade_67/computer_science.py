"""
computer_science.py  (QA Builder — Grade 6/7)
--------------------------------------
...
Key rules:
  ...
  - Computer Science focus: file, folder, software, computer parts
...
- Computer Science focus: observation, definition, component function, procedure steps
"""


class ComputerScienceQA67Builder:

    def __init__(self):
        self.client = anthropic.Anthropic(api_key=settings.ANTHROPIC_API_KEY)
        self.model  = settings.ANTHROPIC_MODEL
        print(f"✅ Computer Science QA Builder (67) v1.0 initialized — model: {self.model}")
    ...
    def generate(self, text: str, metadata: dict) -> Optional[str]:
        ...
        print(f"      [Computer Science QA 67 v1.0] Generating: {lesson_title}")
        parts = []
        ...
        for call_num, (method, label) in enumerate([
            ...
            print(f"      [Computer Science QA 67] Call {call_num}/4: {label}...")
        ...
        if not parts:
            return None
        combined = "\n\n".join(parts)
        print(f"      [Computer Science QA 67 v1.0] ✅ Complete — {len(combined)} chars")
        return combined

    def _call_mcq(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate ONLY MCQ Q1–Q25 for Class 6/7 Computer Science.

{ANSWER_FORMAT_RULES}
{AGE_NOTE}

Chapter: {lesson_title} | Class {class_num} | Unit {unit} | Computer Science
{disc_context}
...
Start Q1. End Q25."""
            ...
            print(f"❌ Computer Science QA 67 MCQ error: {e}")
            return None

    def _call_fill_blanks(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate ONLY Fill Blanks Q26–Q50 for Class 6/7 Computer Science.
...
Start Q26. End Q50."""
            ...
            print(f"❌ Computer Science QA 67 Fill Blanks error: {e}")
            return None
...
Class 6/7 Computer Science. Age-appropriate. No repeats from Q1–Q50.
...
Computer Science Match Pairs:
- Component → Function / Term → Definition / Software → Purpose / Step → Action / Device → Use
...
Start Q51. End Q75. Raw HTML only."""
            ...
            print(f"❌ Computer Science QA 67 Statement+Match error: {e}")
            return None

    def _call_descriptive(self, text, lesson_title, class_num, unit, disc_context, discipline) -> Optional[str]:
        try:
            prompt = f"""Generate 2-mark (Q76–Q88) and 5-mark (Q89–Q100) for Class 6/7 Computer Science.
...
---"""
            ...
            print(f"❌ Computer Science QA 67 Descriptive error: {e}")
            return None


cs_qa_67_builder = ComputerScienceQA67Builder()