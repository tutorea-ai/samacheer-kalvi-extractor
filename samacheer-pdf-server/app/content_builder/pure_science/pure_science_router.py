"""
pure_science_router.py
-----------------------
Router for all Pure Science LP and QA generation.
...
Adding a new grade group:
    1. Create folder:  pure_science/lp/grade_X/ (and pure_science/qa/grade_X/)
    2. Build builders: pure_science/lp/grade_X/biology.py etc.
    3. Add mapping:    _get_grade_group() below

Adding a new discipline:
    (Botany and Zoology already share one file per team decision —
    see pure_science/lp/grade_1112/biology.py module docstring. A genuinely
...
def _get_grade_group(class_num: int) -> Optional[str]:
    ...
    if class_num in [11, 12]:
        ...
        print(f"      [Pure Science Router] ❌ Unknown class: {class_num}")
        return None
    ...
    try:
        if grade_group == "grade_1112":
            if discipline == "botany":
                ...
                print(f"      [Pure Science Router] ❌ Unknown discipline: {discipline}")
                return None

        else:
            print(f"      [Pure Science Router] ❌ Unknown grade group: {grade_group}")
            return None

    except ImportError as e:
        print(f"      [Pure Science Router] ⏳ Builder not yet implemented: {grade_group}/{discipline} — {e}")
        return None
...
# QA BUILDER ROUTER
#
# QA builders now exist (Sept 2026) — pure_science/qa/grade_1112/biology.py.
...
def _get_qa_builder(grade_group: str, discipline: str):
    ...
    try:
        ...
        print(f"      [Pure Science Router] ⏳ QA builder not yet implemented: {grade_group}/{discipline} — {e}")
        return None
...
def generate_lp(text: str, metadata: dict) -> Optional[str]:
    """
    Generate LP HTML for a Pure Science chapter.
    ...
    print(f"      [Pure Science Router] LP request — Class {class_num} | {discipline.title()}")
    ...
    if not builder:
        print(f"      [Pure Science Router] ❌ No LP builder available for {grade_group}/{discipline}")
        return None

    print(f"      [Pure Science Router] → {grade_group}/{discipline} LP builder")
    return builder.generate(text, metadata)


def generate_qa(text: str, metadata: dict) -> Optional[str]:
    """
    Generate QA HTML for a Pure Science chapter.
    ...
    print(f"      [Pure Science Router] QA request — Class {class_num} | {discipline.title()}")
    ...
    if not builder:
        print(f"      [Pure Science Router] ❌ No QA builder available for {grade_group}/{discipline}")
        return None

    print(f"      [Pure Science Router] → {grade_group}/{discipline} QA builder")
    return builder.generate(text, metadata)