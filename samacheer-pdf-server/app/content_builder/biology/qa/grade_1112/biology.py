...
- EXACTLY {tier['count']} questions, Q{tier['start']} through Q{tier['end']}
- Do NOT stop before Q{tier['end']}
...
---"""
            response = self.client.messages.create(
                model=self.model,
                max_tokens=16000,  # highest tier: longest answers + inline
                                    # SVG diagrams for several questions —
                                    # start here given LP's Call 0a lesson
                                    # (measure real usage before trusting
                                    # a lower number)
                system=BIO_QA_SYSTEM_PROMPT,
                messages=[{"role": "user", "content": prompt}],
            )
